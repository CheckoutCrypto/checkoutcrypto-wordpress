jQuery(document).ready(function ($) {
	...
	$("#purchase-btn").attr('href', '#');
	$("#purchase-btn").attr('class', 'lbp-inline-link-1');
...
total_price = $(".total_price").attr('id');
console.log(total_price);
		$.ajax({ 
			...
			data :  {
				action : 'cc_coin',
				coin_code : 	e.target.id.substring(8),	
			...
			success: function(received) {
				...
				if(timeleft > 0) {
					...
					for (var i in arr) 
					{
						html +=" <div class='cc_coin' id='cc_coin_" + arr[i]['coin_code'].toLowerCase() + "' style='background: url("+arr[i]['coin_img']+" ); height: 150px; width: 150px; left no-repeat;'><div id='cc_coin_"+ arr[i]['coin_code'].toLowerCase() + "' class='coin_amount'> Coins: " +arr[i]['coin_total'] + "</div></div>";
					}
				console.log('test2');
					html +=  '</div>';
					...
					onComplete: function() {
						console.log('testcb');
						 $("body").click(function(e) {
							...
							function checkoutcrypto_check (coin_code, total_price, address) {
								if(timeleft > 0) {
									$.ajax({ 
										type: 'POST',
										url: ajaxurl,
								            ...
								            console.log(answer);
										        if(answer['coin_address'] != 0) {
										         ...
										         //  $('#cc_coin_reselect').show(); */
										             ...
										             url_qr_args += '&chld=L';
										             url_qr_args += "&chl="+answer['coin_address'];
										             var url_qr = url_qr_base+url_qr_args;
										             var url_qr_output = '<img src="'+url_qr+'">';

										             document.getElementById("oc-cc-payment-info").innerHTML = 'Please send <span style="font-weight: bold;"> '+answer['coin_amount']+'</span> '+coin_code.toUpperCase()+' to:';
										             document.getElementById("oc-cc-payment-address").value = answer['coin_address'];
										             document.getElementById("oc_cc_payment_qr_address_container").innerHTML = url_qr_output;
										             countdown = setInterval(timer, 1000);
										             checker = setInterval(checkoutcrypto_check(coin_code, answer['coin_amount'], answer['coin_address']), 5);
       												// submit_wp_cc_form(); 
										        }
									}
								});
